...
class WalletWithdrawState(StatesGroup):
    amount = State()
    method = State()
    account = State()
...
async def wallet_deposit_start(q: CallbackQuery, state: FSMContext):
    await state.clear()
    ...
    await safe_edit(
        ...
        reply_markup=K([[('❌ Cancel', 'wallet_cancel')]]),
        parse_mode="HTML",
    )
    await q.answer()


@dp.message(WalletDepositState.amount)
async def wallet_deposit_amount(m: Message, state: FSMContext):
    try:
        amount = dec((m.text or '').strip())
        if amount <= 0 or amount > Decimal('1000000'):
            raise ValueError
    except (InvalidOperation, ValueError):
        return await m.answer("❌ Enter a valid deposit amount.")

    c = con()
    row = c.execute(
        "SELECT value FROM settings WHERE key='min_deposit'"
    ).fetchone()
    c.close()
    minimum = dec(row['value']) if row else Decimal('0')
    if amount < minimum:
        return await m.answer(
            f"❌ Minimum deposit is {minimum} {CURRENCY}."
        )

    await state.update_data(amount=str(amount))
    await state.set_state(WalletDepositState.method)
    await m.answer(
        f"💰 Amount: <b>{amount} {CURRENCY}</b>\n\n"
        "Select payment method:",
        reply_markup=K([
            [("bKash", "wallet_dmethod:bkash"), ("Nagad", "wallet_dmethod:nagad")],
...
@dp.callback_query(F.data.startswith("wallet_dmethod:"))
async def wallet_deposit_method(q: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    if not data.get("amount"):
        await state.clear()
        return await q.answer("Deposit session expired. Start again.", show_alert=True)

    method = q.data.split(":", 1)[1]
    env_name = "BKASH_NUMBER" if method == "bkash" else "NAGAD_NUMBER"
    payment_number = os.getenv(env_name, "CONTACT_ADMIN")

    await state.update_data(method=method)
    await state.set_state(WalletDepositState.trx)
    await safe_edit(
        q.message,
        f"💳 <b>Deposit Instructions</b>\n\n"
        f"Amount: <b>{data['amount']} {CURRENCY}</b>\n"
        f"Method: <b>{method.upper()}</b>\n\n"
        f"Send payment to: <code>{payment_number}</code>\n\n"
        "After payment, send the exact Transaction ID (TrxID):",
        reply_markup=K([[('❌ Cancel', 'wallet_cancel')]]),
        parse_mode="HTML",
    )
    await q.answer()
...
@dp.message(WalletDepositState.trx)
async def wallet_deposit_trx(m: Message, state: FSMContext):
    trx = (m.text or '').strip()
    if len(trx) < 4 or len(trx) > 100:
        return await m.answer("❌ Invalid transaction ID.")
    ...
    try:
        ...
        await state.clear()
        return await m.answer("❌ Deposit session expired. Please start again.")

    c = con()
    existing = c.execute(
        "SELECT id FROM transactions WHERE kind='deposit' AND note LIKE ?",
        (f"%TrxID: {trx}%",),
    ).fetchone()
    if existing:
        c.close()
    ...
    cur = c.execute(
        """
        INSERT INTO transactions(user_id, kind, amount, status, note, created_at)
        VALUES(?,?,?,?,?,?)
        """,
        (
            m.from_user.id,
            ...
            str(amount),
            "pending",
            f"Method: {method.upper()} | TrxID: {trx}",
            now(),
        ),
    )
    tid = cur.lastrowid
    ...
    await m.answer(
        f"✅ <b>Deposit request #{tid} submitted</b>\n\n"
...
async def wallet_withdraw_start(q: CallbackQuery, state: FSMContext):
    register(q.from_user)
    c = con()
    u = c.execute("SELECT balance FROM users WHERE id=?", (q.from_user.id,)).fetchone()
    c.close()
    available = dec(u["balance"]) if u else Decimal('0')
    if available <= 0:
        return await q.answer("❌ Insufficient balance.", show_alert=True)

    await state.clear()
    await state.set_state(WalletWithdrawState.amount)
    await safe_edit(
        q.message,
        f"💸 <b>Withdraw</b>\n\n"
        f"Available balance: <b>{available} {CURRENCY}</b>\n\n"
...
@dp.message(WalletWithdrawState.amount)
async def wallet_withdraw_amount(m: Message, state: FSMContext):
    try:
        amount = dec((m.text or '').strip())
        if amount <= 0:
            raise ValueError
    except (InvalidOperation, ValueError):
        return await m.answer("❌ Enter a valid withdrawal amount.")

    c = con()
    row = c.execute("SELECT value FROM settings WHERE key='min_withdraw'").fetchone()
    ...
    if amount < minimum:
        return await m.answer(f"❌ Minimum withdrawal is {minimum} {CURRENCY}.")
    if amount > available:
        return await m.answer("❌ Insufficient balance.")

    await state.update_data(amount=str(amount))
    await state.set_state(WalletWithdrawState.method)
    await m.answer(
        f"💸 Amount: <b>{amount} {CURRENCY}</b>\n\nSelect withdrawal method:",
        reply_markup=K([
            [("bKash", "wallet_wmethod:bkash"), ("Nagad", "wallet_wmethod:nagad")],
            [("❌ Cancel", "wallet_cancel")],
        ]),
        parse_mode="HTML",
    )
...
@dp.callback_query(F.data.startswith("wallet_wmethod:"))
async def wallet_withdraw_method(q: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    if not data.get("amount"):
        await state.clear()
        return await q.answer("Withdrawal session expired. Start again.", show_alert=True)
    method = q.data.split(":", 1)[1]
    await state.update_data(method=method)
    await state.set_state(WalletWithdrawState.account)
    await safe_edit(
        q.message,
        f"💸 <b>Withdrawal</b>\n\n"
        f"Method: <b>{method.upper()}</b>\n\n"
        "Enter your receiving bKash/Nagad number:",
        reply_markup=K([[('❌ Cancel', 'wallet_cancel')]]),
        parse_mode="HTML",
    )
    await q.answer()
...
@dp.message(WalletWithdrawState.account)
async def wallet_withdraw_account(m: Message, state: FSMContext):
    account = (m.text or '').strip().replace(' ', '').replace('-', '')
    if not account.isdigit() or len(account) not in (10, 11, 12, 13):
        return await m.answer("❌ Enter a valid mobile account number.")
    ...
    try:
        ...
        await state.clear()
        return await m.answer("❌ Withdrawal session expired. Please start again.")

    c = con()
    try:
        c.execute("BEGIN IMMEDIATE")
        u = c.execute("SELECT balance FROM users WHERE id=?", (m.from_user.id,)).fetchone()
        if not u or dec(u['balance']) < amount:
            c.rollback()
            await state.clear()
            return await m.answer("❌ Insufficient balance. Please try again.")

        new_balance = dec(u['balance']) - amount
        c.execute("UPDATE users SET balance=? WHERE id=?", (str(new_balance), m.from_user.id))
        cur = c.execute(
            """
            INSERT INTO transactions(user_id, kind, amount, status, note, created_at)
            VALUES(?,?,?,?,?,?)
            ...
            (
                ...
                str(amount),
                "pending",
                f"Method: {method.upper()} | Account: {account}",
                now(),
            ),
        )
        tid = cur.lastrowid
        ...
        c.close()
        await state.clear()
        return await m.answer("❌ Could not create withdrawal request. Try again.")
    c.close()
    await state.clear()

    await m.answer(
        f"✅ <b>Withdrawal request #{tid} submitted</b>\n\n"
        f"Amount: {amount} {CURRENCY}\n"
        f"Method: {method.upper()}\n"
        f"Account: <code>{account}</code>\n\n"
        "⏳ Amount reserved. Admin will review your request.",
        reply_markup=wallet_kb(),
        parse_mode="HTML",
    )
    ...
    if ADMIN_ID:
        try:
            await bot.send_message(
                ...
                reply_markup=K([
                    [("✅ Paid", f"wallet_wd:paid:{tid}"), ("❌ Reject", f"wallet_wd:reject:{tid}")]
                ]),
                parse_mode="HTML",
            )
        except Exception:
...
async def wallet_history(q: CallbackQuery):
    register(q.from_user)
    c = con()
    rows = c.execute(
        """
        SELECT id, kind, amount, status, note, created_at
        FROM transactions
        WHERE user_id=? AND kind IN ('deposit','withdrawal')
    ...
    for r in rows:
        icon = "➕" if r['kind'] == 'deposit' else "💸"
        lines.append(
            f"{icon} #{r['id']} • {r['amount']} {CURRENCY} • {r['status']}\n"
            f"{r['note'] or ''}"
        )
...
@dp.callback_query(F.data == "wallet_cancel")
async def wallet_cancel(q: CallbackQuery, state: FSMContext):
    await state.clear()
    await safe_edit(
        q.message,
...
@dp.callback_query(F.data.startswith("wallet_wd:"))
async def wallet_withdraw_admin_action(q: CallbackQuery):
    if q.from_user.id != ADMIN_ID:
        return await q.answer("Unauthorized", show_alert=True)

    _, action, tid_raw = q.data.split(":")
    tid = int(tid_raw)
    c = con()
    try:
        c.execute("BEGIN IMMEDIATE")
        tx = c.execute(
            "SELECT * FROM transactions WHERE id=? AND kind='withdrawal' AND status='pending'",
            (tid,),
        ).fetchone()
        if not tx:
            c.rollback()
        ...
        if action == "reject":
            ...
            new_balance = dec(u['balance']) + dec(tx['amount'])
            c.execute("UPDATE users SET balance=? WHERE id=?", (str(new_balance), tx['user_id']))
            c.execute("UPDATE transactions SET status='rejected', note=note || ' | Rejected by admin' WHERE id=?", (tid,))
            user_text = f"❌ Withdrawal #{tid} rejected. {tx['amount']} {CURRENCY} returned to your wallet."
        else:
            c.execute("UPDATE transactions SET status='completed', note=note || ' | Paid by admin' WHERE id=?", (tid,))
            user_text = f"✅ Withdrawal #{tid} has been paid: {tx['amount']} {CURRENCY}."
        c.commit()
    except Exception:
        c.rollback()
        c.close()
        return await q.answer("Processing failed.", show_alert=True)
    c.close()

    await q.message.edit_reply_markup(reply_markup=None)
    await q.answer("Processed")
    try:
